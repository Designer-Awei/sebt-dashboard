...
BLE Notify (NimBLE-Arduino)
    ↓ [BLE无线通信]
浏览器 Web Bluetooth API
    ↓ [WebSocket客户端]
Electron WebSocket服务器 (端口3000)
    ↓ [IPC事件: bluetooth-data]
app.js (渲染进程/前端)
    ↓ [DOM更新]
用户界面 (8方向传感器网格)
```
...
**3. bt-manager.js 接收并解析：**
```javascript
// 接收Buffer: <Buffer 39 30 00 00 03 96 00 F4 01 ...>
const sensorData = {
  timestamp: 12345,
  minDirection: 3,
  minDistance: 150,
...
1. **通信架构**：ESP32-C3 BLE → 浏览器Web Bluetooth API → WebSocket → Electron → UI
2. **数据格式**：23字节固定长度二进制数据包，通过BLE Notify发送
3. **字节序**：所有多字节数据使用小端序（Little Endian）
4. **发送频率**：每300ms发送一次（仅在BLE客户端连接时）
5. **方向映射**：0-7对应8个方向，255表示无效
6. **数据解析**：浏览器端解析二进制数据，转换为JSON格式
7. **实时更新**：WebSocket实时转发，前端立即更新8方向显示
...
在 `app.js` `handleWebSocketData` 中添加：
```javascript
console.log('前端处理传感器数据:', sensorData);
```

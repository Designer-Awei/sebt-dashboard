# SEBT 数据流程详解

## 📊 数据流程图

### 主机数据流
```
ESP32-C3主机 (硬件端)
    ↓ [23字节二进制数据包]
BLE Notify (NimBLE-Arduino)
    ↓ [BLE无线通信]
浏览器 Web Bluetooth API (ble-driver.html)
    ↓ [WebSocket客户端]
Electron WebSocket服务器 (端口3000)
    ↓ [IPC事件: bluetooth-data-received]
ble-manager.js (主进程)
    ↓ [数据验证和转换]
    ↓ [IPC事件: bluetooth-data-received]
app.js (渲染进程/前端)
    ↓ [handleHostBroadcast处理]
    ↓ [DOM更新]
用户界面 (8方向传感器网格)
```

### 从机数据流（计划中）
```
ESP32-C3从机 (硬件端)
    ↓ [JSON格式: {pressure:3000}]
BLE Notify (NimBLE-Arduino)
    ↓ [BLE无线通信]
浏览器 Web Bluetooth API (ble-driver.html - 从机区域，计划集成)
    ↓ [WebSocket客户端]
Electron WebSocket服务器 (端口3000)
    ↓ [IPC事件: slave-pressure-data-received]
app.js (渲染进程/前端)
    ↓ [压力稳定性判断]
    ↓ [updatePressureDisplay更新中心卡片显示]
    ↓ [测距按钮状态更新]
用户界面 (中心卡片显示pressure：xxxx，测距按钮启用/禁用)
```

**注意**：
1. 测距数据收集直接使用`handleHostBroadcast`中的第一手数据，不使用WebSocket传递的第二手数据
2. 自动锁定功能只在实验运行状态（`experimentRunning = true`）时执行
3. 已完成测距的方向会被排除在锁定逻辑和绿色实时高亮计算之外
4. **从机数据**：从机负责控制"测距按钮"是否可用，需要同时满足稳定阈值和稳定时长参数

---

## 1️⃣ 硬件端发送的数据格式

### 数据包结构（23字节二进制）

```cpp
// 位置  大小    内容              说明
// 0-3   4字节   时间戳            millis()的值，小端序
// 4     1字节   最小方向索引       0-7表示方向，255表示无效
// 5-6   2字节   最小距离           uint16_t，小端序，单位mm
// 7-22  16字节  8方向距离数组      每个方向2字节，共8个方向
```

### BLE通信特性
- **协议**: BLE GATT Notify
- **Service UUID**: `0000aaaa-0000-1000-8000-00805f9b34fb`
- **Characteristic UUID**: `0000bbbb-0000-1000-8000-00805f9b34fb`
- **发送间隔**: 300ms（有客户端连接时）
- **数据可靠性**: 硬件只在BLE客户端连接时发送数据

### 实际数据示例

假设当前状态：
- 时间戳：`12345` (0x00003039)
- 最小方向：`3` (Front方向)
- 最小距离：`150mm` (0x0096)
- 8方向距离：`[500, 600, 400, 150, 800, 700, 450, 550]`

**二进制数据包（十六进制）：**
```
39 30 00 00    // 时间戳: 12345 (小端序)
03              // 最小方向: 3 (Front)
96 00           // 最小距离: 150mm (小端序)
F4 01           // 方向0: 500mm
58 02           // 方向1: 600mm
90 01           // 方向2: 400mm
96 00           // 方向3: 150mm (最小距离)
20 03           // 方向4: 800mm
BC 02           // 方向5: 700mm
C2 01           // 方向6: 450mm
2E 02           // 方向7: 550mm
```

**完整23字节：**
```
39 30 00 00 03 96 00 F4 01 58 02 90 01 96 00 20 03 BC 02 C2 01 2E 02
```

### 方向映射

```cpp
const char* DIR_NAMES[8] = {
  "Left",         // 0 → L
  "BackLeft",    // 1 → BL
  "FrontLeft",   // 2 → FL
  "Front",        // 3 → F
  "Back",         // 4 → B
  "BackRight",   // 5 → BR
  "FrontRight",  // 6 → FR
  "Right"         // 7 → R
};
```

### 发送频率

- **发送间隔**：每300ms发送一次
- **数据包大小**：固定23字节
- **通信协议**：BLE GATT Notify
- **连接要求**：只有在BLE客户端连接时才发送数据

---

## 2️⃣ 软件端接收和处理

### WebSocket Bridge 架构

新的架构采用了WebSocket Bridge设计，将BLE通信与Electron应用解耦：

```
ESP32-C3 → BLE Notify → 浏览器Web Bluetooth API → WebSocket → Electron → UI
```

#### 步骤1：BLE设备连接（浏览器端）

```javascript
// public/ble-driver.html
await navigator.bluetooth.requestDevice({
  filters: [{ services: [SERVICE_UUID] }]
});

const server = await device.gatt.connect();
const service = await server.getPrimaryService(SERVICE_UUID);
const characteristic = await service.getCharacteristic(CHARACTERISTIC_UUID);
  
// 启用通知接收数据
await characteristic.startNotifications();
characteristic.addEventListener('characteristicvaluechanged', handleData);
```

#### 步骤2：WebSocket数据转发

```javascript
// 接收BLE数据并通过WebSocket转发
function handleCharacteristicValueChanged(event) {
  const data = event.target.value;
  const dataView = new DataView(data.buffer);

  // 解析23字节数据包
  const sensorData = {
    type: 'sensor_data',
    timestamp: dataView.getUint32(0, true),
    minDirection: dataView.getUint8(4),
    minDistance: dataView.getUint16(5, true),
    distances: Array.from({length: 8}, (_, i) =>
      dataView.getUint16(7 + i * 2, true))
  };

  // 通过WebSocket发送到Electron
  sendToElectron(sensorData);
}
```

#### 步骤3：Electron WebSocket服务器

```javascript
// main.js - WebSocket服务器
wss.on('connection', (ws) => {
  ws.on('message', (message) => {
    const data = JSON.parse(message.toString());
  
    // 转发到渲染进程
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('bluetooth-data', data);
    }
  });
});
```

#### 步骤3：BLE管理器数据处理（主进程）

```javascript
// ble-manager.js - 接收WebSocket数据
handleWebSocketData(data) {
  if (data.type === 'sensor_data') {
    const sensorData = this.validateSensorData(data);
    if (sensorData) {
      this.processSensorData(sensorData);
    }
  }
}

// 数据验证（浏览器已解析，直接验证JSON数据）
validateSensorData(data) {
  const isValid = data.minDistance >= 0 && data.minDistance <= 2000 &&
                  data.timestamp > 0 && data.minDirection >= -1 &&
                  data.minDirection < 8 && Array.isArray(data.distances) &&
                  data.distances.length === 8;

  return isValid ? data : null;
}

// 转换为前端格式并发送
sendSensorData(sensorData) {
  // 转换为前端需要的格式：[[dir, dist], [dir, dist], ...]
  const distances = sensorData.distances.map((dist, index) => [index, dist]);
  
  const payload = {
    source: 'host',
    name: 'SEBT-Host-001',
    address: this.device?.address || 'unknown',
    timestamp: sensorData.timestamp,
    distances,  // [[0,500], [1,600], ...]
    minDir: sensorData.minDirection,
    minDist: sensorData.minDistance,
    currentMinDirection: sensorData.minDirection,
    currentMinDistance: sensorData.minDistance,
    lockedDirection: -1,
    pressure: null
  };
  
  // 通过IPC发送到渲染进程
  this.sendToRenderer('bluetooth-data-received', {
    type: 'scan_data',
    data: JSON.stringify(payload)
  });
}
```

#### 步骤4：前端数据处理（渲染进程）

```javascript
// app.js - 接收和处理主机广播数据
ipcRenderer.on('bluetooth-data-received', (event, data) => {
  if (data.type === 'scan_data') {
    const payload = JSON.parse(data.data);
    if (payload.source === 'host') {
      // 主机传感器数据，直接处理（第一手数据）
      this.handleHostBroadcast(payload);
    }
  }
});

// handleHostBroadcast - 统一处理硬件端数据格式
handleHostBroadcast(payload) {
  // 数据格式：distances是[[dir, dist], [dir, dist], ...]格式
  // 这是第一手数据，直接使用真实传入数据，不进行有效性判断
  
  // 1. 更新传感器数据
  payload.distances.forEach(([dir, dist]) => {
    // 直接使用传入的真实数据
    distancesArray[dir] = dist;
    this.updateSensorData(dir, dist, 'hardware');
    
    // 更新UI显示（排除正在测距和已完成的方向）
    if (!this.bluetoothMeasurementCollection || 
        this.bluetoothMeasurementCollection.channel !== dir) {
      if (!this.completedDirections.has(dir)) {
        this.updateSensorDisplay(dir, sensorData);
      }
    }
  });
  
  // 2. 构建过滤后的距离数组（排除已完成的方向）
  const filteredDistancesArray = hasCompletedDirections 
    ? distancesArray.map((d, idx) => 
        this.completedDirections.has(idx) ? Infinity : d
      )
    : distancesArray;
  
  // 3. 高亮最近方向（排除已完成和已锁定的方向）
  this.highlightClosestDirection(filteredDistancesArray);
  
  // 4. 检查并执行自动锁定（只在实验运行且无锁定方向时）
  if (!hasLockedDirection && this.experimentRunning) {
    // 计算最小方向（排除已完成的方向）
    let minDir = payload.currentMinDirection ?? payload.minDir;
    let minDist = payload.currentMinDistance ?? payload.minDist;
    
    if (minDir === undefined || minDir === -1 || minDir === 255) {
      // 从过滤后的数组中计算
      let calcMin = Infinity;
      let calcDir = -1;
      filteredDistancesArray.forEach((d, idx) => {
        if (typeof d === 'number' && isFinite(d) && d >= 0 && d < calcMin) {
          calcMin = d;
          calcDir = idx;
        }
      });
      minDir = calcDir;
      minDist = calcMin;
    }
    
    // 异步执行自动锁定检查（不阻塞数据更新）
    if (minDir >= 0 && minDir < 8 && 
        typeof minDist === 'number' && isFinite(minDist) && 
        !this.completedDirections.has(minDir)) {
      requestAnimationFrame(() => {
        this.checkAutoLock(minDir, minDist);
      });
    }
  }
  
  // 5. 处理测距数据收集（如果正在收集）
  if (this.bluetoothMeasurementCollection) {
    const { channel, direction, distances, maxSamples } = this.bluetoothMeasurementCollection;
    const collectedDistance = distancesArray[channel];
    
    // 直接使用第一手数据进行收集
    if (typeof collectedDistance === 'number' && 
        isFinite(collectedDistance) && collectedDistance > 0) {
      distances.push(collectedDistance);
      
      // 检查是否收集够了样本
      if (distances.length >= maxSamples) {
        const averageDistance = Math.round(
          distances.reduce((sum, dist) => sum + dist, 0) / distances.length
        );
        this.handleManualMeasurementResult(channel, averageDistance, direction);
        this.cancelBluetoothMeasurementCollection();
      }
    }
  }
}
```

**转换后的JSON数据示例：**
```json
{
  "source": "host",
  "name": "SEBT-Host-001",
  "address": "BLE_DEVICE_ID",
  "timestamp": 12345,
  "distances": [
    [0, 500],   // 方向0: Left, 500mm
    [1, 600],   // 方向1: BackLeft, 600mm
    [2, 400],   // 方向2: FrontLeft, 400mm
    [3, 150],   // 方向3: Front, 150mm ← 最小距离
    [4, 800],   // 方向4: Back, 800mm
    [5, 700],   // 方向5: BackRight, 700mm
    [6, 450],   // 方向6: FrontRight, 450mm
    [7, 550]    // 方向7: Right, 550mm
  ],
  "minDir": 3,
  "minDist": 150,
  "currentMinDirection": 3,
  "currentMinDistance": 150,
  "lockedDirection": -1,
  "pressure": null
}
```

---

## 3️⃣ 前端展示流程

### app.js 接收和处理

#### 步骤1：IPC事件监听

```javascript
// app.js:setupIPCListeners()
ipcRenderer.on('bluetooth-data-received', (event, data) => {
  if (data.type === 'scan_data') {
    const payload = JSON.parse(data.data);
    if (payload.source === 'host') {
      // 主机传感器数据，直接处理（第一手数据）
      this.handleHostBroadcast(payload);
    }
  }
});
```

#### 步骤2：处理主机广播数据（统一数据格式）

```javascript
// app.js - 处理主机广播的8方向数据
handleHostBroadcast(payload) {
  // 数据格式统一：distances是[[dir, dist], [dir, dist], ...]格式
  // 这是来自ble-manager.js的第一手数据，用于所有数据处理
  
  const distancesArray = new Array(8).fill(this.INVALID_DISTANCE);
  
  // 处理8方向距离数据
  payload.distances.forEach(([dir, dist]) => {
    const processedDist = (dist === 2000 || !this.isValidDistance(dist)) 
      ? this.INVALID_DISTANCE 
      : dist;
    distancesArray[dir] = processedDist;
    this.updateSensorData(dir, processedDist, 'hardware');
  });
  
  // 高亮最近方向
  this.highlightClosestDirection(distancesArray);
  
  // 处理测距数据收集（直接使用第一手数据）
  if (this.bluetoothMeasurementCollection) {
    const { channel } = this.bluetoothMeasurementCollection;
    const collectedDistance = distancesArray[channel];
    // 收集最近3次数据并计算平均值
  }
  
  // 检查并执行自动锁定
  this.checkAutoLock(minDir, minDist);
}
```

#### 步骤3：更新UI显示

```javascript
// app.js - 更新传感器显示
updateSensorDisplay(sensorData) {
  const gridElement = this.gridElements.get(sensorData.direction);
  if (!gridElement) return;

  const distanceElement = gridElement.querySelector('.distance-display');
  
  // 更新距离文本
  distanceElement.textContent = sensorData.distance > 0
    ? `${sensorData.distance} mm`  // 例如: "150 mm"
    : '--- mm';
  
  // 更新样式
  if (sensorData.isMinDistance) {
    // 最小距离方向：橙色高亮
    gridElement.classList.add('min-distance');
    distanceElement.style.color = '#f59e0b';
  } else {
    // 普通方向：蓝色显示
    gridElement.classList.remove('min-distance');
    distanceElement.style.color = '#3b82f6';
  }
}
```

#### 步骤4：高亮最小距离方向

```javascript
// app.js - 更新最小距离高亮
updateMinDistanceHighlight(minDirection) {
  // 清除所有高亮
  this.gridElements.forEach((element) => {
    element.classList.remove('min-distance');
  });
  
  // 找到最小距离的传感器
  let minDistance = Infinity;
  let minChannel = -1;
  
  this.sensorData.forEach((data, channel) => {
    if (data.distance > 0 && data.distance < minDistance) {
      minDistance = data.distance;
      minChannel = channel;
    }
  });
  
  // 高亮最小距离方向
  if (minChannel >= 0) {
    const minElement = this.gridElements.get(minChannel);
    minElement.classList.add('min-distance');
  }
}
```

---

## 4️⃣ 前端UI布局

### 8方向传感器网格

```
        [FL] FrontLeft
            ↑
[L] Left ← [F] Front → [R] Right
            ↓
        [BL] BackLeft

    [B] Back
```

**HTML结构：**
```html
<div class="sensor-grid">
  <!-- 方向0: Left -->
  <div class="sensor-item" data-channel="0">
    <div class="direction-label">L</div>
    <div class="distance-display">500 mm</div>
  </div>
  
  <!-- 方向1: BackLeft -->
  <div class="sensor-item" data-channel="1">
    <div class="direction-label">BL</div>
    <div class="distance-display">600 mm</div>
  </div>
  
  <!-- ... 其他6个方向 ... -->
  
  <!-- 方向3: Front (最小距离) -->
  <div class="sensor-item min-distance" data-channel="3">
    <div class="direction-label">F</div>
    <div class="distance-display" style="color: #3b82f6;">150 mm</div>
  </div>
</div>
```

### 视觉反馈

1. **实时数据**：蓝色文字 (`#3b82f6`)
2. **最小距离方向**：添加 `min-distance` 类，可能有边框高亮
3. **锁定状态**：绿色文字 (`#10b981`)，添加 `active` 类

---

## 5️⃣ 完整数据流示例

### 场景：检测到前方150mm处有物体

**1. ESP32-C3 读取传感器：**
```cpp
// master-bt.ino:238-264
distances = [500, 600, 400, 150, 800, 700, 450, 550];
currentMinDir = 3;  // Front
currentMinDist = 150;
timestamp = 12345;
```

**2. 构造并发送23字节数据包：**
```cpp
// master-bt.ino:266-286
uint8_t dataPacket[23] = {
  0x39, 0x30, 0x00, 0x00,  // timestamp: 12345
  0x03,                     // minDir: 3
  0x96, 0x00,               // minDist: 150
  0xF4, 0x01,               // dist[0]: 500
  0x58, 0x02,               // dist[1]: 600
  // ... 其他6个方向
};
bluetoothSerial.write(dataPacket, 23);
```

**3. ble-manager.js 接收并解析：**
```javascript
// 接收WebSocket数据（来自浏览器）
const sensorData = {
  timestamp: 12345,
  minDirection: 3,
  minDistance: 150,
  distances: [500, 600, 400, 150, 800, 700, 450, 550]  // 硬件端原始格式
};

// 转换为前端格式
const distances = sensorData.distances.map((dist, index) => [index, dist]);
// distances = [[0,500], [1,600], [2,400], [3,150], ...]
```

**4. 通过IPC发送到前端：**
```javascript
// IPC事件: 'bluetooth-data-received'
{
  type: 'scan_data',
  data: '{"source":"host","timestamp":12345,"distances":[[0,500],[1,600],...],"currentMinDirection":3,"currentMinDistance":150}'
}
```

**5. app.js handleHostBroadcast处理：**
```javascript
// 解析payload
const payload = JSON.parse(data.data);
// payload.distances = [[0,500], [1,600], [2,400], [3,150], ...]

// 处理每个方向的数据
payload.distances.forEach(([dir, dist]) => {
  this.updateSensorData(dir, dist, 'hardware');
});

// 测距数据收集（如果正在收集）
if (this.bluetoothMeasurementCollection) {
  const collectedDistance = distancesArray[channel];
  // 直接使用第一手数据进行收集
}
```

**5. app.js 更新UI：**
```javascript
// 更新8个方向的显示
方向0 (Left):     "500 mm" (蓝色)
方向1 (BackLeft): "600 mm" (蓝色)
方向2 (FrontLeft): "400 mm" (蓝色)
方向3 (Front):    "150 mm" (蓝色 + min-distance高亮) ← 最小距离
方向4 (Back):     "800 mm" (蓝色)
方向5 (BackRight): "700 mm" (蓝色)
方向6 (FrontRight): "450 mm" (蓝色)
方向7 (Right):    "550 mm" (蓝色)
```

---

## 📝 关键要点总结

1. **通信架构**：ESP32-C3 BLE → 浏览器Web Bluetooth API → WebSocket → Electron主进程(ble-manager.js) → 渲染进程(app.js) → UI
2. **数据格式**：
   - 硬件端：23字节固定长度二进制数据包
   - 软件端：统一使用`[[dir, dist], [dir, dist], ...]`格式（由ble-manager.js转换）
3. **字节序**：所有多字节数据使用小端序（Little Endian）
4. **发送频率**：每300ms发送一次（仅在BLE客户端连接时）
5. **方向映射**：0-7对应8个方向，255表示无效
6. **数据解析**：浏览器端解析二进制数据，ble-manager.js转换为统一格式
7. **数据有效性**：软件端直接使用真实的传入数据，不进行有效性判断
8. **测距数据收集**：直接使用`handleHostBroadcast`中的第一手数据，不使用WebSocket传递的第二手数据
9. **实时更新**：数据通过IPC实时传递，前端立即更新8方向显示
10. **自动锁定**：只在实验运行状态（`experimentRunning = true`）时执行，排除已完成的方向
11. **高亮互斥**：绿色实时高亮和蓝色锁定高亮完全互斥，不会同时显示
12. **已完成方向排除**：使用标记数组方式，确保已完成的方向完全排除在锁定逻辑和绿色高亮计算之外

---

## 🔍 调试技巧

### 查看BLE设备连接

在浏览器控制台查看：
```javascript
// 检查BLE支持
navigator.bluetooth.getAvailability()

// 查看已连接设备
navigator.bluetooth.getDevices()
```

### 查看WebSocket通信

在浏览器Network标签页查看WebSocket连接，或在控制台添加：
```javascript
// WebSocket消息监听
ws.addEventListener('message', (event) => {
  console.log('WebSocket收到:', JSON.parse(event.data));
});
```

### 查看数据解析过程

在 `public/ble-driver.html` 中 `handleCharacteristicValueChanged` 函数内添加：
```javascript
console.log('BLE原始数据:', Array.from(new Uint8Array(value.buffer)));
console.log('解析后数据:', sensorData);
```

### 查看Electron端接收

在 `main.js` WebSocket消息处理中添加：
```javascript
console.log('Electron收到WebSocket数据:', data);
```

### 查看前端处理

在 `app.js` `handleHostBroadcast` 中添加：
```javascript
console.log('前端处理主机广播数据:', payload);
console.log('距离数组:', distancesArray);
```

